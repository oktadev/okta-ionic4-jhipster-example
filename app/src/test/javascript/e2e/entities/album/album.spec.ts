...
describe('Album e2e test', () => {
  ...
  it('should load Albums', async () => {
    ...
    expect(await albumComponentsPage.getTitle()).to.eq('oauth2App.album.home.title');
  });

  it('should load create Album page', async () => {
    await albumComponentsPage.clickOnCreateButton();
    albumUpdatePage = new AlbumUpdatePage();
    expect(await albumUpdatePage.getPageTitle()).to.eq('oauth2App.album.home.createOrEditLabel');
  ...
  it('should delete last Album', async () => {
    ...
    expect(await albumDeleteDialog.getDialogTitle()).to.eq('oauth2App.album.delete.question');
